...
// using Strassen's algorithm to reduce the number of expensive `big.Int`
// multiplications from 8 to 7.
//
// Strassen's algorithm involves the following steps:
// 1. Calculate 10 sums and differences of elements from the input matrices.
// 2. Compute 7 matrix products (the recursive step in the original algorithm).
// 3. Combine these products through addition and subtraction to get the result.
//
// This implementation is optimized for 2x2 matrices of `big.Int`s and can
// perform the 7 multiplications in parallel.
