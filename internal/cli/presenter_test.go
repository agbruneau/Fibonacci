...
// orchestration interfaces it is documented to implement.

func TestCLIResultPresenter_InterfaceCompliance(t *testing.T) {
	t.Parallel()
	var (
		_ orchestration.ResultPresenter = cli.CLIResultPresenter{}
		_ orchestration.ErrorHandler    = cli.CLIResultPresenter{}
	)
}
...
func TestCLIResultPresenter_PresentComparisonTable(t *testing.T) {
	...
	cli.CLIResultPresenter{}.PresentComparisonTable(results, &buf)
	out := buf.String()
...
func TestCLIResultPresenter_PresentResult(t *testing.T) {
	...
	var buf bytes.Buffer
	cli.CLIResultPresenter{}.PresentResult(res, 8, false, false, true, &buf)
	// Adapter success is "produces any output"; the content is pinned down
	// by display_test.go's TestDisplayResult* cases.
	if buf.Len() == 0 {
...
func TestCLIResultPresenter_HandleError(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			var buf bytes.Buffer
			p := cli.CLIResultPresenter{MachineOutput: tc.machineOutput}
