...
This section provides the canonical mapping of Go concurrency and error patterns to their idiomatic Rust equivalents.

| Go Pattern               | Rust Equivalent                                    | Crate           |
| :----------------------- | :------------------------------------------------- | :-------------- |
| `sync.WaitGroup`         | `tokio::task::JoinSet`                             | `tokio`         |
| `context.Context`        | `CancellationToken` + `tokio::select!`             | `tokio_util`    |
| `chan<- T`               | `tokio::sync::mpsc::Sender<T>`                     | `tokio`         |
| `sync.Pool`              | `crossbeam::queue::SegQueue<Box<T>>`               | `crossbeam`     |
| `sync.Mutex`             | `tokio::sync::Mutex` (async) or `std::sync::Mutex` | `tokio` / `std` |
| `promauto.NewCounterVec` | `prometheus::register_int_counter_vec!`            | `prometheus`    |
| `zerolog`                | `tracing` with `tracing-subscriber`                | `tracing`       |
| `errors.New()`           | `thiserror::Error` derive macro                    | `thiserror`     |
| `fmt.Errorf()`           | `anyhow::Context`                                  | `anyhow`        |
| `panic()`                | Return `Result<T, E>` (no panics in lib code)      | N/A             |

## Error Types

### CalculationError

- **Definition**: The domain error type for calculation failures.
- **Code Rule**:
  ```rust
  ...
  pub enum CalculationError {
      ...
      Cancelled,
      #[error("index {0} exceeds maximum supported value")]
      IndexTooLarge(u64),
...
- **Definition**: The application-level error type for HTTP handlers.
- **Context**: Used in `axum` handlers to convert domain errors to HTTP responses.
- **Code Rule**:
