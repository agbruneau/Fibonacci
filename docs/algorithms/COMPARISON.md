...
> **Note**: The implemented Strassen-Winograd variant (`internal/fibonacci/matrix_ops.go`) keeps 7 multiplications and uses 15 additions/subtractions total (7 adds + 8 subs) — fewer than the textbook Strassen (18 add/sub). It still trades more add/sub for fewer multiplications, so it pays off once one entry-sized product costs more than the extra additions — between 512 and 1024 bits on the one host measured, well below the 3072-bit default ([CALIBRATION.md § Where the Defaults Come From](../CALIBRATION.md#where-the-defaults-come-from)).
